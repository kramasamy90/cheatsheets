...
\textbf{Determinant: }\\
Any function $f(A): \mathbb{R}^{n\times n} \to \mathbb{R}$ satisfying the following three basic properties.
...
\begin{enumerate}
	...
	Proof:\\
    Say $A = E_1\cdots E_k U$, then $AB = E_1\cdots E_k UB$\\
    For a simple row operation matrix $E_i$, $\det(AB) = \det(UB)$\\
    $\det(UB) = \prod_iu_{ii}det(B)$ and $\det(A) = \prod_{i}u_{ii}$.
	\item $det(A^T) = det(A)$.\\
	Proof:\\ 
