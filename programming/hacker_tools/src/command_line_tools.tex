...
\begin{tabularx}{\linewidth}{lX}
    ...
    % Disk usage.
    \texttt{du}                     & Disk usage, i.e., size of blocks allocated.\\ 
    \texttt{-s}                     & Summarize, show total only.\\
    \texttt{-h}                     & Human-readable sizes.\\
    \texttt{-b}                     & Actual file size in bytes.\\
                                    & \texttt{--apparent-size} like \texttt{-b}, but output in Kib and not bytes.\\
...
\begin{tabularx}{\linewidth}{lX}
    ...
    \texttt{tar}            & Tape archive. Combine with a compression flag to handle \texttt{.tar.gz}/\texttt{.tgz}, \texttt{.tar.bz2}, \texttt{.tar.xz}.\\
    \texttt{-c}             & Create archive.\\
    \texttt{-x}             & Extract archive.\\
    \texttt{-t}             & List contents.\\
    \texttt{-v}             & Verbose.\\
    \texttt{-f}             & Use archive file (must be last flag, followed by filename).\\
    \texttt{-z}             & Filter through \texttt{gzip}, for \texttt{.tar.gz} / \texttt{.tgz}.\\
    \texttt{-j}             & Filter through \texttt{bzip2}, for \texttt{.tar.bz2}.\\
    \texttt{-J}             & Filter through \texttt{xz}, for \texttt{.tar.xz}.\\
    \hline
\end{tabularx}
...
\begin{tabularx}{\linewidth}{lX}
    ...
    \texttt{gzip} / \texttt{gunzip}         &    Compress a single file to \texttt{.gz}, \texttt{.bz2}, \texttt{.xz} resp. \\
    \texttt{bzip2} / \texttt{bunzip2}       &    (\texttt{tar} a directory first). \\
    \texttt{xz} / \texttt{unxz}             & \texttt{gzip foo.tar} $\to$ \texttt{foo.tar.gz} (replaces file).\\
                                            & \texttt{gzip -c foo.tar > foo.tar.gz}.\\
                                            & \texttt{gunzip foo.tar.gz} $\to$ \texttt{foo.tar}.\\
    \hline


\end{tabularx}
    
\textcolor{red}{TODO: chown, chgrp, (un)compress, zgrep, zcat, zless, zdiff.}\\
