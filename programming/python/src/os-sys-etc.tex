...
\begin{tabularx}{\linewidth}{lX}
    ...
    \hline
    \texttt{expanduser()}    & Expand \texttt{\char`~} and \texttt{\char`~user} to the user's home directory.\\
                             & Eg: \texttt{os.path.expanduser('\char`~/foo')}\\
                             & $\rightarrow$ \texttt{/home/user/foo}.\\
    \texttt{expandvars()}    & Expand environment variables like \texttt{\$HOME}.\\
                             & Eg: \texttt{os.path.expandvars('\$HOME/foo')}\\
                             & $\rightarrow$ \texttt{/home/user/foo}.\\
    \hline
\end{tabularx}
